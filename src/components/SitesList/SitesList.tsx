...
export const SitesList: React.FC<ISitesListProps> = React.memo(({
  ...
  const favoriteSitesForSorting = React.useMemo((): Set<string> => {
    return displayFavoriteSites !== undefined ? displayFavoriteSites : favoriteSites;
  ...
  const filteredSites = React.useMemo((): ISite[] => {
    ...
    return sites.filter((site: ISite): boolean => siteMatchesSearch(site, searchText));
  }, [sites, searchText]);
   ...
   */
  const { sortedSites, favoriteCount } = React.useMemo((): { sortedSites: ISite[]; favoriteCount: number } => {
    ...
    const sortedFavoriteSites: ISite[] = sortSitesAlphabetically(favoriteSitesList);
   ...
   */
  const highlightText = React.useCallback((text: string): React.ReactElement => {
    ...
    return (
      <div style={sitesListContainerStyles}>
        <div 
          ...
          aria-atomic="true"
        >
          <Spinner size={SpinnerSize.medium} aria-label={UI_MESSAGES.LOADING_SITES} />
