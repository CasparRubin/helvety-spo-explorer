 ...
 */
// eslint-disable-next-line @rushstack/no-new-null -- DOM API returns null, not undefined
export function isFocusableElement(element: Element | null): element is HTMLElement & { focus: () => void } {
  return (
    element !== null &&
...
export function querySearchInput(): HTMLInputElement | null {
  ...
  for (const selector of selectors) {
    ...
    try {
      ...
      logWarning(LOG_SOURCE, extractErrorMessage(queryError), `Invalid selector: ${selector}`);
      continue;
    }
  }
...
export function safeFocus(
  ...
  if (!element) {
    logWarning(LOG_SOURCE, 'Element is null or undefined', context ?? 'safeFocus');
    return false;
  }

  // Validate element has focus method
  if (!isFocusableElement(element)) {
    logWarning(LOG_SOURCE, 'Element does not have focus method', context ?? 'safeFocus');
    return false;
  }
...
export function safeFocusDelayed(
  ...
  if (typeof delayMs !== 'number' || !Number.isFinite(delayMs) || delayMs < 0) {
    logWarning(LOG_SOURCE, `Invalid delayMs: ${delayMs}`, context ?? 'safeFocusDelayed');
    // Use 0 as fallback
    delayMs = 0;
  }
  ...
  if (typeof setTimeout === 'undefined') {
    logWarning(LOG_SOURCE, 'setTimeout is not available', context ?? 'safeFocusDelayed');
    return undefined;
  }
  ...
  try {
    ...
    return (): void => {
      try {
        ...
        logWarning(LOG_SOURCE, extractErrorMessage(clearError), 'Failed to clear timeout');
      }
    };
  } catch (timeoutError: unknown) {
    logWarning(LOG_SOURCE, extractErrorMessage(timeoutError), context ?? 'safeFocusDelayed');
    return undefined;
  }
}
