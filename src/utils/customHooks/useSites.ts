...
export function useSites(
  ...
  React.useEffect((): void => {
    try {
      ...
      logError(
        LOG_SOURCE,
        err,
        "Error initializing SiteService - sites will not be available"
      );
  ...
  const fetchSites = React.useCallback(async (): Promise<void> => {
    if (!siteServiceRef.current) {
      ...
      logError(
        ...
        "fetchSites - service not available"
      );
      ...
      async (): Promise<void> => {
        ...
        if (!isValidSitesArray(fetchedSites)) {
          ...
          logError(
            LOG_SOURCE,
            new Error(errorMessage),
            "fetchSites - invalid response format"
          );
          setError(errorMessage);
          setSites([]);
          return;
        ...
        const currentWebUrl: string =
          contextRef.current.pageContext.web.absoluteUrl.toLowerCase();
        ...
        setSelectedSite(
          (prevSelected: ISite | undefined): ISite | undefined => {
            ...
            if (currentSite && isValidSite(currentSite)) {
              ...
              return prevSelected?.id === currentSite.id
                ? prevSelected
                : currentSite;
            }
      ...
      timeoutId = setTimeout((): void => {
        ...
        void fetchSites().catch((err: unknown): void => {
          logError(
            LOG_SOURCE,
            err,
            "Unhandled promise rejection in fetchSites"
          );
  ...
  const selectSite = React.useCallback((site: ISite): void => {
    ...
    if (!isValidSite(site)) {
      logError(
        LOG_SOURCE,
        new Error("Invalid site provided to selectSite"),
