...
export function useFavorites(userId: string): IUseFavoritesReturn {
  const [favoriteSites, setFavoriteSites] = React.useState<Set<string>>(
    new Set()
  );

  // Initialize favorite service using reusable hook
  const { serviceRef: favoriteServiceRef } =
    useServiceInitialization<FavoriteService>({
  ...
  const refreshFavorites = React.useCallback((): void => {
    if (!favoriteServiceRef.current) {
      logError(
        LOG_SOURCE,
        new Error("FavoriteService not initialized"),
        "Cannot refresh favorites - service not available"
      );
      return;
    }
  ...
  const toggleFavorite = React.useCallback(
    (siteUrl: string): void => {
      ...
      if (!isNonEmptyString(siteUrl)) {
        logError(
          LOG_SOURCE,
          new Error("Invalid site URL provided"),
          `Cannot toggle favorite - invalid URL: ${siteUrl}`
        );
        return;
      }
      ...
      if (!favoriteServiceRef.current) {
        logError(
          LOG_SOURCE,
          new Error("FavoriteService not initialized"),
          "Cannot toggle favorite - service not available"
        );
        return;
      }
