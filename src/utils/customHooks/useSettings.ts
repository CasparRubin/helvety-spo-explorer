...
export function useSettings(userId: string): IUseSettingsReturn {
  const [settings, setSettings] = React.useState<IUserSettings>({
    ...DEFAULT_SETTINGS,
  });

  // Initialize settings service using reusable hook
  const { serviceRef: settingsServiceRef } =
    useServiceInitialization<SettingsService>({
      ...
      onInitialized: (service: SettingsService): void => {
        ...
        setTimeout((): void => {
          ...
          if (!isValidUserSettings(userSettings)) {
            logError(
              LOG_SOURCE,
              new Error("Invalid settings loaded from storage"),
              `User: ${userId}`
            );
  ...
  const updateSettings = React.useCallback(
    (newSettings: IUserSettings): void => {
      ...
      if (!isPlainObject(newSettings) || !isValidUserSettings(newSettings)) {
        logError(
          LOG_SOURCE,
          new Error("Invalid settings object provided"),
          "Cannot update settings - invalid settings object"
        );
        return;
      }
      ...
      if (!settingsService) {
        logError(
          LOG_SOURCE,
          new Error("SettingsService not initialized"),
          "Cannot update settings - service not available"
        );
        return;
      }
