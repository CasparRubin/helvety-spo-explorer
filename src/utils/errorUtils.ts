...
export function isError(error: unknown): error is Error {
  ...
  const hasName: boolean =
    typeof errorLike.name === "string" && errorLike.name.length > 0;
...
export function isApiError(error: unknown): error is ApiError {
  ...
  const hasApiErrorProperties: boolean =
    hasValidStatusCode || hasValidApiEndpoint;
...
export function isPermissionError(error: unknown): error is PermissionError {
  ...
  const errorWithCategory = error as Error & {
    category?: ErrorCategory | unknown;
  };
...
export function isValidationError(error: unknown): error is ValidationError {
  ...
  const hasValidField: boolean = typeof field === "string" && field.length > 0;
...
function parseHttpStatusFromMessage(message: string): number | undefined {
  const httpStatusMatch: RegExpMatchArray | null = message.match(
    /\b(40[0-9]|50[0-9])\b/
  );
  if (httpStatusMatch) {
 ...
 * @returns Error category if found, undefined otherwise
 */
function getErrorCategoryFromCustomError(
  error: unknown
 ...
 */
function categorizeErrorByHttpStatus(
  errorMessage: string
): ErrorCategory | undefined {
  const httpStatus: number | undefined =
    parseHttpStatusFromMessage(errorMessage);
 ...
 */
function categorizeErrorByKeywords(
  errorMessage: string
): ErrorCategory | undefined {
