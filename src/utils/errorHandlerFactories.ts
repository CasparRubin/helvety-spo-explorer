...
import { createStandardizedError, formatErrorContext } from './errorHandlingUtils';
...
export function handleSharePointApiError(
  ...
  const operationContext: string = formatErrorContext('getSites - SharePoint API failed', undefined, `Site: ${siteUrl}, Web: ${webUrl}`);
...
/**
 * Handles Microsoft Graph API errors and throws appropriate error types
 * 
 * Standardizes error handling for Microsoft Graph API failures, creating
 * appropriate error types based on error category. Graph API errors have
 * a specific structure: { error: { code: string, message: string } }
 * 
 * @deprecated This function is kept for backward compatibility but is no longer used.
 * Use handleSharePointApiError instead for SharePoint Search API calls.
 * 
 * @param error - The error that occurred
 * @param context - Application customizer context for URL extraction
 * @param logSource - Log source identifier
 * @param apiUrl - The Graph API URL that was called
 * @param defaultErrorMessage - Default error message if parsing fails
 * @throws ApiError, PermissionError, or ValidationError based on error category
 * 
 * @example
 * ```typescript
 * try {
 *   await getSitesFromGraph();
 * } catch (error) {
 *   handleGraphApiError(error, appContext, 'SiteService', graphUrl, 'Failed to fetch sites');
 * }
 * ```
 */
export function handleGraphApiError(
  error: unknown,
  context: { pageContext: { web: { absoluteUrl: string }; site: { absoluteUrl: string } } },
  logSource: string,
  apiUrl: string,
  ...
  const webUrl: string = context.pageContext.web.absoluteUrl;
  const errorContext: string = formatErrorContext(
    'getSites - Graph API failed',
    undefined,
    `Category: ${errorCategory}. Site URL: ${siteUrl}, Web URL: ${webUrl}. Graph API URL: ${apiUrl}. Error: ${errorMessage}`
  );
  
  logError(logSource, error, errorContext);
  
  const operationContext: string = formatErrorContext('getSites - Graph API failed', undefined, `Site: ${siteUrl}, Web: ${webUrl}`);
  
  // Throw appropriate custom error based on category with user-friendly messages
  if (errorCategory === ErrorCategory.PERMISSION) {
    throw new PermissionError(
      `Unable to fetch sites from Microsoft Graph. Please check your permissions (Sites.Read.All) and try again.`,
      error,
      operationContext
    );
  } else if (errorCategory === ErrorCategory.NETWORK) {
    throw new ApiError(
      `Unable to fetch sites from Microsoft Graph. Please check your network connection and try again.`,
      undefined,
      apiUrl,
      error,
      operationContext
