 ...
 * It uses a single SharePoint Search API request capped at 500 rows to fetch sites.
 * Results are cached for 5 minutes to improve performance.
 *
 * Features:
 * - Single-request fetch capped to 500 results for reliability
...
export class SiteService {
  ...
  /**
   * Get up to 500 sites the current user has access to
   * Uses SharePoint Search API to fetch sites in a single request
   * Results are cached for 5 minutes to improve performance
   *
   * Caching strategy:
  ...
  public async getSites(): Promise<ISite[]> {
    ...
    try {
      ...
      const staleCachedSites: ISite[] | null =
        this.cache && isValidSitesArray(this.cache) ? this.cache : null;
      ...
      handleSharePointApiError(
        ...
        LOG_SOURCE,
        searchUrl,
        ERROR_MESSAGES.FETCH_SITES_FAILED
      );
    }
  }
   ...
   * ```
   */
  private handleSearchApiResponseError(
    ...
    if (response.status === 401 || response.status === 403) {
      logError(
        LOG_SOURCE,
        new Error(errorText),
        `SharePoint Search API permission error (${response.status}). URL: ${searchUrl}`
      );
    ...
    logError(
      LOG_SOURCE,
      new Error(errorText),
      `SharePoint Search API request failed (${response.status}). URL: ${searchUrl}`
    );
    throw new ApiError(
   ...
   */
  private filterValidSearchResults(
    rows: readonly ISiteSearchResultRow[]
  ): ISiteSearchResultRow[] {
   ...
   */
  private mapSearchResultsToSites(
    validRows: readonly ISiteSearchResultRow[]
  ): ISite[] {
    return validRows.map(
      (row: ISiteSearchResultRow): ISite => this.mapSearchResultToSite(row)
    );
  }
  ...
  private logProcessingSummary(rawCount: number, finalCount: number): void {
    ...
    if (finalCount === 0 && rawCount > 0) {
      ...
      logError(
        LOG_SOURCE,
        new Error(warningDetails),
        "getSitesFromSearch - all sites filtered out"
      );
      ...
      logWarning(
        LOG_SOURCE,
        `${filteredCount} sites were filtered out during processing`,
        `from ${rawCount} total to ${finalCount} valid`
      );
    }
  }
  ...
  private processSearchResults(rows: readonly ISiteSearchResultRow[]): ISite[] {
    ...
    const validSearchResults: ISiteSearchResultRow[] =
      this.filterValidSearchResults(rows);
    logInfo(
      LOG_SOURCE,
      `${validSearchResults.length} sites passed isValidSearchResult validation`,
      `out of ${rawSiteCount} total`
    );
    ...
    const mappedSites: ISite[] =
      this.mapSearchResultsToSites(validSearchResults);
    logInfo(
      LOG_SOURCE,
      `${mappedSites.length} sites mapped to ISite format`,
      ""
    );
    ...
    logInfo(
      LOG_SOURCE,
      `${finalSites.length} sites passed final validation`,
      `out of ${mappedSites.length} mapped sites`
    );
   ...
   */
  private createSearchRequestPayload(queryText: string): {
    ...
    try {
      ...
      if (
        ...
        logError(
          LOG_SOURCE,
          apiError,
          `SharePoint Search API permission error (401/403). URL: ${searchUrl}`
        );
        throw new PermissionError(
          ERROR_MESSAGES.FETCH_SITES_PERMISSIONS,
          apiError,
   ...
   *
   * Uses the SharePoint Search API /_api/search/postquery endpoint to retrieve
   * up to SEARCH_QUERY_PARAMS.ROW_LIMIT sites the current user has access to.
  ...
  private async getSitesFromSearch(): Promise<ISite[]> {
    ...
    const searchUrl: string = `${this.context.pageContext.web.absoluteUrl}${API_ENDPOINTS.SEARCH_POSTQUERY}`;
    const requestBody = this.createSearchRequestPayload(
    ...
    logInfo(
      LOG_SOURCE,
      `SharePoint Search API returned ${rows.length} raw rows`,
      totalRows !== undefined
        ? `reported total rows: ${totalRows}, fetch cap: ${SEARCH_QUERY_PARAMS.ROW_LIMIT}`
        : `reported total rows: unavailable, fetch cap: ${SEARCH_QUERY_PARAMS.ROW_LIMIT}`
    );

    if (
      typeof totalRows === "number" &&
      totalRows > SEARCH_QUERY_PARAMS.ROW_LIMIT
    ) {
      logWarning(
        LOG_SOURCE,
        `Search returned more than ${SEARCH_QUERY_PARAMS.ROW_LIMIT} total rows (${totalRows}). Only the first ${SEARCH_QUERY_PARAMS.ROW_LIMIT} rows are fetched.`,
        "getSitesFromSearch - capped result set"
      );
    }
  ...
  private isValidSearchResult(row: ISiteSearchResultRow): boolean {
    ...
    const hasPath: boolean = row.Cells.some(
      (cell) =>
        cell.Key === "Path" &&
        typeof cell.Value === "string" &&
        cell.Value.length > 0
    );
    const hasTitle: boolean = row.Cells.some(
      (cell) => cell.Key === "Title" && typeof cell.Value === "string"
    );
